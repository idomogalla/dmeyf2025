...
tryCatch(
  {
    ...
    dataset_train_eval <- dataset[foto_mes %in% PARAM$evaluacion$training]

    # Undersampling
    log_info("Haciendo undersampling para el modelo final.")
    set.seed(PARAM$semilla_primigenia, kind = "L'Ecuyer-CMRG")
    dataset_train_eval[, azar := runif(nrow(dataset_train_eval))]
    dataset_train_eval[, training := 0L]
    ...
    # Agregar Canaritos al dataset de entrenamiento
    log_info(paste("Agregando", PARAM$qcanaritos, "canaritos al dataset de evaluación"))
    ...
    campos_buenos_z <- setdiff(
      colnames(dataset_train_eval),
      PARAM$trainingstrategy$campos_entrenar
    )

    log_info("Creando dtrain con canaritos.")
    dtrain <- lgb.Dataset(
      data = data.matrix(dataset_train_eval[training == 1L, campos_buenos_z, with = FALSE]),
      label = dataset_train_eval[training == 1L, clase01],
      free_raw_data = FALSE
    )
    ...
    dfuture <- dataset[foto_mes %in% PARAM$evaluacion$future]

    if (nrow(dfuture) == 0) {
      stop(paste("No se encontraron datos para el período de testing:", PARAM$evaluacion$future))
    }
    ...
    cortes_evaluacion <- PARAM$evaluacion$cortes_evaluacion
    ...
    log_info(paste0(
      ...
      " (Iter: ", PARAM$evaluacion$iter,
      " x kSemillerio: ", PARAM$evaluacion$ksemillerio, ")"
    ))
    ...
    PARAM$evaluacion$envios_optimos_promedio <- envios_optimos_vector
  ...
  error = function(e) {
    log_error("######################################################")
    log_error("Se ha producido un error fatal en la Sección 8: Evaluación.")
