tryCatch(
  {
    ...
    file_probabilidades <- file.path(PARAM$experimento_folder, paste0("prediccion_probabilidades_final_", PARAM$experimento, ".csv"))
    ...
    if (isTRUE(PARAM$train_final$produccion)) {
      ...
      if (!file.exists(ruta_envios_rds)) {
        stop(paste(
          "No se encontró el archivo de envíos óptimos:", ruta_envios_rds,
          "\nAsegúrate de que 8_Evaluacion.R se haya ejecutado correctamente."
        ))
      }
  ...
  error = function(e) {
    log_error("######################################################")
    log_error("Se ha producido un error fatal en la Sección 10: Modelo Final.")
