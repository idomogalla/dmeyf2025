...
PARAM <- list()
PARAM$experimento <- "expC01_Prueba16_3"
...
tryCatch({  
  ...
  dataset_train_final <- dataset[foto_mes %in% PARAM$train]
  dtrain_final_kaggle <- lgb.Dataset(data = data.matrix(dataset_train_final[, campos_buenos, with = FALSE]), label = dataset_train_final[, clase01])
  ...
  GenerarEnviosKaggle(
    ...
    envios_optimos = c(10500, 11000, 11200),
...
tryCatch({
  ...
  dataset_train_final_ens <- dataset[foto_mes %in% PARAM$train]
