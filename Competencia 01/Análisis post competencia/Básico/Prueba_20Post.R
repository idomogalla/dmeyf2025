...
tryCatch({
  ...
  for (semilla_actual in PARAM$semillas_ensemble) {
    log_info(paste0("--- Iniciando BO y re-entrenamiento para semilla: ", semilla_actual, " en datos de Kaggle ---"))

    # Seteamos la semilla para la BO
    PARAM$lgbm$param_fijos$seed <- semilla_actual

    # Definimos la función de estimación para la BO en el dataset de Kaggle
    EstimarGanancia_AUC_lightgbm_kaggle <- function(x) {
      param_completo <- modifyList(PARAM$lgbm$param_fijos, x)
      # Usamos dtrain_kaggle que tiene todos los datos
      modelocv <- lgb.cv(data = dtrain_kaggle, nfold = PARAM$hyperparametertuning$xval_folds, stratified = TRUE, param = param_completo)
      AUC <- modelocv$best_score
      rm(modelocv)
      gc(full = TRUE, verbose = FALSE)
      log_info(paste("Iteración BO Kaggle [Semilla:", semilla_actual, "] -> AUC:", format(AUC, digits = 6), "|", format(Sys.time(), "%X")))
      return(AUC)
    }

    # Directorio y archivo específicos para esta BO de Kaggle
    dir_semilla_kaggle <- file.path(PARAM$carpeta_bayesiana, paste0("semilla_kaggle_", semilla_actual))
    dir.create(dir_semilla_kaggle, showWarnings = FALSE, recursive = TRUE)
    kbayesiana_kaggle <- file.path(dir_semilla_kaggle, "bayesiana_kaggle.RDATA")

    funcion_optimizar_kaggle <- EstimarGanancia_AUC_lightgbm_kaggle
    configureMlr(show.learner.output = FALSE)
    obj.fun_kaggle <- makeSingleObjectiveFunction(fn = funcion_optimizar_kaggle, minimize = FALSE, noisy = TRUE, par.set = PARAM$hyperparametertuning$hs, has.simple.signature = FALSE)
    ctrl_kaggle <- makeMBOControl(save.on.disk.at.time = 600, save.file.path = kbayesiana_kaggle)
    ...
    # surr.km ya está definido antes, lo podemos reusar

    if (!file.exists(kbayesiana_kaggle)) {
      log_info(paste("Iniciando nueva búsqueda Bayesiana de Kaggle de", PARAM$hyperparametertuning$iteraciones, "iteraciones para semilla", semilla_actual))
      bayesiana_salida_kaggle <- mbo(obj.fun_kaggle, learner = surr.km, control = ctrl_kaggle)
    } else {
      log_info(paste("Continuando búsqueda Bayesiana de Kaggle desde archivo para semilla", semilla_actual))
      bayesiana_salida_kaggle <- mboContinue(kbayesiana_kaggle)
    }

    log_info(paste("Optimización Bayesiana de Kaggle finalizada para la semilla:", semilla_actual))

    tb_bayesiana_kaggle <- as.data.table(bayesiana_salida_kaggle$opt.path)
    setorder(tb_bayesiana_kaggle, -y)
    
    mejores_hiperparametros_kaggle <- tb_bayesiana_kaggle[1, setdiff(colnames(tb_bayesiana_kaggle), c("y", "dob", "eol", "error.message", "exec.time", "ei", "error.model", "train.time", "prop.type", "propose.time", "se", "mean", "iter")), with = FALSE]
    
    log_info(paste("Mejores hiperparámetros de Kaggle para semilla", semilla_actual, ":"))
    log_info(paste(capture.output(print(mejores_hiperparametros_kaggle)), collapse = ""))
    log_info(paste("Mejor AUC (y) de Kaggle para semilla", semilla_actual, ":", tb_bayesiana_kaggle[1, y]))

    # Entrenar el modelo final para esta semilla con los hiperparámetros recién encontrados
    param_final_kaggle <- modifyList(PARAM$lgbm$param_fijos, mejores_hiperparametros_kaggle)
    param_final_kaggle$seed <- semilla_actual
    param_final_kaggle$early_stopping_round <- NULL # No se usa en el entrenamiento final

    modelo_kaggle <- lgb.train(data = dtrain_kaggle, param = param_final_kaggle)
