...
tryCatch({  
  ...
  # Nueva optimización bayesiana para el modelo final de Kaggle
  log_info("Iniciando nueva Optimización Bayesiana para el modelo final de Kaggle.")
  
  EstimarGanancia_AUC_lightgbm_final_kaggle <- function(x) {
    param_completo <- modifyList(PARAM$lgbm$param_fijos, x)
    modelocv <- lgb.cv(
  ...
  kbayesiana_final_kaggle <- paste0(PARAM$carpeta_bayesiana, "bayesiana_final_kaggle.RDATA")
  funcion_optimizar_final <- EstimarGanancia_AUC_lightgbm_final_kaggle
  
  obj.fun_final <- makeSingleObjectiveFunction(
    fn = funcion_optimizar_final,
    minimize = FALSE,
    noisy = TRUE,
    par.set = PARAM$hyperparametertuning$hs,
    has.simple.signature = FALSE
  ...
  ctrl_final <- makeMBOControl(save.on.disk.at.time = 600,
                               save.file.path = kbayesiana_final_kaggle)
  ctrl_final <- setMBOControlTermination(ctrl_final, iters = PARAM$hyperparametertuning$iteraciones)
  ctrl_final <- setMBOControlInfill(ctrl_final, crit = makeMBOInfillCritEI())

  if (!file.exists(kbayesiana_final_kaggle)) {
    log_info(
      paste(
        "Iniciando nueva búsqueda Bayesiana (final) de",
        PARAM$hyperparametertuning$iteraciones,
        "iteraciones."
      )
    )
    bayesiana_salida_final <- mbo(obj.fun_final, learner = surr.km, control = ctrl_final)
  } else {
    log_info("Continuando búsqueda Bayesiana (final) desde archivo existente.")
    bayesiana_salida_final <- mboContinue(kbayesiana_final_kaggle)
  }
  log_info("Optimización Bayesiana (final) finalizada.")

  tb_bayesiana_final <- as.data.table(bayesiana_salida_final$opt.path)
  setorder(tb_bayesiana_final, -y)
  
  mejores_hiperparametros_final <- tb_bayesiana_final[1, setdiff(
    colnames(tb_bayesiana_final),
    c("y", "dob", "eol", "error.message", "exec.time", "ei", "error.model", "train.time", "prop.type", "propose.time", "se", "mean", "iter")
  ), with = FALSE]
  
  param_final_kaggle <- modifyList(PARAM$lgbm$param_fijos, mejores_hiperparametros_final)
  param_final_kaggle$early_stopping <- NULL
  param_final_kaggle$early_stopping_round <- NULL
  param_final_kaggle$early_stopping_rounds <- NULL

  log_info("Mejores hiperparámetros encontrados para el modelo final de Kaggle:")
  log_info(paste(capture.output(print(mejores_hiperparametros_final)), collapse = "
"))
  log_info(paste("Mejor AUC (y) para el modelo final:", tb_bayesiana_final[1, y]))

  modelo_final_kaggle <- lgb.train(data = dtrain_final_kaggle, param = param_final_kaggle)
...
tryCatch({
  ...
  for (semilla_actual in PARAM$semillas_ensemble) {
    ...
    modelo_final_ens <- lgb.train(data = dtrain_final_kaggle_ens, param = param_final_kaggle)
