...
PARAM <- list()
PARAM$experimento <- "expC01_Prueba09"
...
PARAM$train_final <- c(202101, 202102) # Meses para el entrenamiento final (post-BO)
...
PARAM$hyperparametertuning$hs <- makeParamSet(
  ...
  makeNumericParam("learning_rate",      lower= 0.02,   upper= 0.4),
  makeNumericParam("feature_fraction",   lower= 0.35,    upper= 0.85),
  makeIntegerParam("num_leaves",         lower= 16L,    upper= 3000L),
  makeIntegerParam("min_data_in_leaf",   lower= 20000L, upper= 100000L) 
)
...
# Función para crear el dataset cuántico
crear_dataset_cuantico <- function(d) {
  # Registros clase01==0 (CONTINUA) se quedan como están
  # Registros clase01==1 (POSITIVOS) se mantienen
  # Se agrega una copia de los clase01==1 PERO con clase01=0
  positivos <- d[clase01 == 1L]
  gemelos_negados <- copy(positivos)
  gemelos_negados[, clase01 := 0L]

  # Unimos el dataset original con los gemelos negados
  log_info(paste0(
    "Dataset cuántico creado. Se agregaron ",
    nrow(gemelos_negados),
    " registros gemelos."
  ))
  return(rbind(d, gemelos_negados))
}
...
tryCatch({
  ...
  log_info("Preparación de datos finalizada. El dataset para la BO se creará en cada iteración.")

}, error = function(e) {
  log_error("######################################################")
  log_error("Se ha producido un error fatal en la Sección 4: Preparación de Datos.")
...
tryCatch({
  ...
  # <MODIFICACION TOTAL DE LA FUNCION>
  EstimarGanancia_AUC_lightgbm <- function(x) {
    gc(full = TRUE, verbose = FALSE)
    # Combinar parámetros fijos con los de la iteración actual de BO
    param_completo <- modifyList(PARAM$lgbm$param_fijos, x)

    # 1. Preparar datos de ENTRENAMIENTO para esta iteración de BO
    dtrain_bo_original <- dataset[foto_mes %in% PARAM$bo_train]

    # 2. Crear la versión CUÁNTICA del dataset de entrenamiento
    dtrain_bo_cuantico <- crear_dataset_cuantico(dtrain_bo_original)

    # 3. Aplicar undersampling (si está definido)
    set.seed(PARAM$semilla_primigenia)
    dtrain_bo_cuantico[, azar := runif(nrow(dtrain_bo_cuantico))]
    filas_training <- dtrain_bo_cuantico[
      azar <= PARAM$trainingstrategy$undersampling | clase01 == 1L
      ]

    # 4. Crear lgb.Dataset de entrenamiento
    dtrain_lgb <- lgb.Dataset(
      data = data.matrix(filas_training[, campos_buenos, with = FALSE]),
      label = filas_training[, clase01],
      free_raw_data = FALSE
    )

    # 5. Preparar datos de VALIDACIÓN para esta iteración
    dvalid_bo <- dataset[foto_mes %in% PARAM$bo_validation]
    dvalid_lgb <- lgb.Dataset(
      data = data.matrix(dvalid_bo[, campos_buenos, with = FALSE]),
      label = dvalid_bo[, clase01],
      free_raw_data = FALSE
    )

    # 6. Entrenar el modelo usando train/validation en lugar de CV
    modelo_bo <- lgb.train(
      data = dtrain_lgb,
    ...
    # 7. Obtener el mejor AUC de la validación
  ...
  fwrite(tb_bayesiana, file = paste0(PARAM$carpeta_bayesiana, "BO_log.txt"), sep = "\t")
...
tryCatch({
  ...
  log_info("Creando dataset cuántico para el entrenamiento final.")
  dataset_train_original <- dataset[foto_mes %in% PARAM$train_final]
  dataset_train_cuantico <- crear_dataset_cuantico(dataset_train_original)
  
  dtrain_final <- lgb.Dataset(
    data = data.matrix(dataset_train_cuantico[, campos_buenos, with = FALSE]),
    label = dataset_train_cuantico[, clase01]
  )
  ...
  log_info("Entrenamiento del modelo final (cuántico) completado.")
