...
PARAM <- list()
PARAM$experimento <- "expC01_Prueba15"
...
tryCatch({
  ...
  # Ajusto aguinaldo según mes anterior
  # Encadenamos todas las operaciones en un solo bloque para mayor eficiencia y claridad.
  dataset[, `:=` (
    # Paso A: Crear columnas de lag temporales.
    # Usamos shift() para obtener el valor de mpayroll del mes anterior (lag 1) y de dos meses antes (lag 2).
    # Lo hacemos "by = numero_de_cliente" para que el cálculo se reinicie para cada cliente.
    mpayroll_lag_1 = shift(mpayroll, n = 1, type = "lag"),
    mpayroll_lag_2 = shift(mpayroll, n = 2, type = "lag")
  ), by = .(numero_de_cliente)][, `:=` ( # Encadenamos la siguiente operación
    # Paso B: Crear una bandera temporal para identificar el aguinaldo.
    # Esta bandera será 1 si se cumplen todas las condiciones, de lo contrario será 0.
    # Las condiciones son:
    # 1. El mes es Junio de 2021 (202106).
    # 2. El payroll actual es al menos un 30% mayor que el del mes pasado.
    # 3. El payroll actual es al menos un 30% mayor que el de hace dos meses.
    # Añadimos !is.na() para evitar problemas con los primeros meses de cada cliente donde los lags son NA.
    flag_aguinaldo = fcase(
          foto_mes == 202106 &
          mpayroll / mpayroll_lag_1 >= 1.3 &
          #mpayroll / mpayroll_lag_2 >= 1.3 &
          !is.na(mpayroll_lag_1) & !is.na(mpayroll_lag_2),
        1L,
      default = 0L
    )
  )][, `:=` ( # Encadenamos la siguiente operación
    # Paso C: Modificar las columnas mpayroll y cpayroll_trx usando la bandera.
    # Usamos ifelse() para aplicar el cambio solo en las filas donde la bandera es 1.
    
    # Si flag_aguinaldo es 1, mpayroll toma el valor del mes anterior (el lag 1 que ya calculamos).
    # Si no, mpayroll se queda con su valor original.
    mpayroll = ifelse(flag_aguinaldo == 1, mpayroll_lag_1, mpayroll),
    
    # Si flag_aguinaldo es 1 y cpayroll_trx es mayor a 1, se le resta 1.
    # En cualquier otro caso, cpayroll_trx se queda con su valor original.
    cpayroll_trx = ifelse(flag_aguinaldo == 1 & cpayroll_trx > 1, cpayroll_trx - 1, cpayroll_trx)
  )][, `:=` ( # Encadenamos la operación final
    # Paso D: Eliminar las columnas auxiliares que ya no necesitamos.
    # Asignar NULL a un nombre de columna en data.table la elimina eficientemente.
    mpayroll_lag_1 = NULL,
    mpayroll_lag_2 = NULL,
    flag_aguinaldo = NULL
  )]
