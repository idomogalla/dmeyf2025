...
GraficarCurvasEnsemble <- function(lista_resultados, PARAM_plot) {
  ...
  labels_individuales <- sprintf("S %s: G %s (E %s)", maximos_individuales$semilla, format(maximos_individuales$ganancia_total, big.mark = ".", decimal.mark = ","), maximos_individuales$clientes)
  label_promedio <- sprintf("Promedio: G %s (E %s)", format(maximo_promedio$ganancia_total, big.mark = ".", decimal.mark = ","), maximo_promedio$clientes)
  ...
  p <- ggplot() +
    geom_line(data = tb_todas, aes(x = clientes, y = ganancia_total, group = semilla, color = semilla), alpha = 0.4, linewidth = 0.8) +
    ...
    geom_label(data = maximo_promedio, aes(x = clientes, y = ganancia_total, label = paste0("Máximo\n", format(ganancia_total, big.mark = ".", decimal.mark = ","), "\n", format(clientes, big.mark = ".", decimal.mark = ","), " Envíos")), vjust = -0.5, fill = "white", color = "red", fontface = "bold") +
    scale_y_continuous(labels = scales::comma, expand = expansion(mult = c(0.05, 0.1))) + 
    scale_x_continuous(labels = scales::comma) + 
    scale_color_manual(name = "Modelo", values = colores_plot, labels = labels_plot) +
    labs(title = paste0("Ganancia Acumulada (Semillas y Ensamble) - ", PARAM_plot$experimento), x = "Clientes Contactados (Envíos)", y = "Ganancia Acumulada") +
    theme_minimal() +
  ...
  ggsave(ruta_grafico, plot = p, width = 12, height = 8)
