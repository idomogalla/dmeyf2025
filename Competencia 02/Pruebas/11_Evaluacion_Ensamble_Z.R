...
# (Función GraficarImportancia sin cambios)
...
# (Función GraficarCurvasEnsemble sin cambios)
...
tryCatch({
  ...
  if(is.null(PARAM$eval_ensamble$iter)) PARAM$eval_ensamble$iter <- 1L
  if(is.null(PARAM$eval_ensamble$APO)) PARAM$eval_ensamble$APO <- FALSE
  if(is.null(PARAM$eval_ensamble$ksemillerio)) {
      log_error("PARAM$eval_ensamble$ksemillerio no está definido. Asumiendo 10.")
      PARAM$eval_ensamble$ksemillerio <- 10
  }
  ...
  for (semilla_actual in semillas_a_evaluar) {
    ...
    # --- INICIO DE MODIFICACIÓN: Añadir columna 'semilla' ---
    ...
    # --- INICIO DE MODIFICACIÓN: rm() más seguro ---
    # (No eliminamos tb_pred_individual ni resultados_individual)
  ...
  # 4. Evaluación del Ensamble Promediado (ESTRATEGIA "GRAN ENSAMBLE")
...
{
type: uploaded file
fileName: 11_Evaluacion_Ensamble_Z.R
fullContent:
#--- Funciones de Evaluación ---
particionar <- function(data, division, agrupa = "", campo = "fold", start = 1, seed = NA) {
  if (!is.na(seed)) set.seed(seed, "L'Ecuyer-CMRG")
...
El script 10 se llama 10_Optimizacion_Bayesiana.R
El script 11 se llama 11_Evaluacion_Ensamble.R
El script 12 se llama 12_Modelo_Final.R

El error que te pase antes sigue ocurriendo en 11_Evaluacion_Ensamble.R. Que ademas es el script que te paso por duplicado.

El script 11_Evaluacion_Ensamble_Z.R, el cual no te lo pase, es el que tiene las mismas correcciones pero que se adaptan a la logica _Z, por lo que te pido que me pases el 11_Evaluacion_Ensamble_Z.R con las correcciones que me pasaste antes