...
# 
plocal$qcanaritos <- as.integer( args[2] )
plocal$min_data_in_leaf <- as.integer( args[3] )
plocal$learning_rate <- as.numeric(  args[4] )
plocal$gradient_bound <- as.numeric(  args[5] )
plocal$num_iterations <- as.numeric(  args[6] )
...
PARAM <- list()
PARAM$experimento <- paste0("apo-s", args[1])
...
PARAM$train_final$lgbm$param_completo <-  list(
  ...
  num_iterations= plocal$num_iterations,
